...
use phpseclib3\Crypt\EC\BaseCurves\Base;
 ...
 echo base_url('pro_get'); ?>`;
  ...
  <a href="<?= base_url('product/wishlist'); ?>" 
    ...
    <!-- Wishlist Icon -->
    <img src="<?= base_url('image/banner/my-wish-list.png'); ?>" 
         alt="Wishlist" 
         class="w-8 h-8 transition-transform transform hover:scale-110 
                group-hover:grayscale group-hover:sepia group-hover:hue-rotate-330">
		...
		<script>
			$('#sortopation').on('change', function() {
				...
				$('.grid').on('click', '.con', function() {
					...
					console.log(pid);
					window.location.href = '<?php echo base_url('product/Usershow/'); ?>' + pid;
				});
...
<div class="w-full py-12">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-[#e5effc] rounded-2xl pl-6 text-white flex flex-col lg:flex-row justify-between hover:scale-[1.02] transition duration-300">
            <!-- Text Section -->
            <div class="w-full md:w-1/2 max-w-xl py-8">
            ...
            <div class="w-full md:w-[500px] h-full">
                <img
                    src=<?php echo base_url("./image/banner/image-(4).png")?>
                    alt="Hero Woman"
        ...
        <div id="cartdata" class="fixed bottom-6 right-6 z-50">
            <button class="relative bg-black text-white p-3 rounded-full shadow-lg hover:scale-110 transition">
                ...
                <span class="absolute -top-1 -right-1 bg-red-500 text-white text-xs font-bold px-1.5 py-0.5 rounded-full"><?php echo count($cart1) ?></span>
            </button>
        </div>
	...
	$(document).ready(function () {
		...
		// Fix: base_url must be in quotes
		let base_url = "<?= base_url('image/multi/') ?>";
		let wishlistData = <?= json_encode($cart); ?>;
		let currentIndex = 0;
		...
		function showWishlistItem() {
			if (wishlistData.length > 0) {
				let item = wishlistData[currentIndex];
				let wishlistHtml = `
					<div class="relative w-[350px] h-[270px] rounded-lg overflow-hidden shadow-md flex items-end justify-center bg-cover bg-center"
					     style="background-image: url('${base_url}${item.pro_image}')">
						<div class="bg-black bg-opacity-60 w-full text-center py-2">
							<span class="text-white font-semibold text-sm">Cart</span>
							<span class="text-white font-semibold text-sm">${item.pro_name}</span>
							<div class="mt-1">
								<span class="text-gray-300 line-through text-xs">Rs.2999</span>
								<span class="text-white font-bold text-sm ml-2">Rs. ${item.pro_price || item.price}</span>
				...
				$("#wishlist-slider").off("click").on("click", function () {
					window.location.href = "<?= base_url('US/') ?>" + item.pro_id;
				});
			...
			$base_url = base_url('image/sccategory/');
			 foreach ($sc as $item): ?>
				<div class="scategory flex flex-col items-center  p-4 border rounded-lg hover:shadow-md transition" data-id="<?= $item['sc_id']; ?>">
					...
					<img src="<?= $base_url.$item['sc_image']; ?>" alt="<?= $item['sc_name']; ?>" class="object-contain w-20 h-20" />
...
<div class="w-full rounded-xl shadow overflow-x-auto  p-4 mt-10">
    <h2 class="text-xl font-bold mb-4">50% Off Products</h2>

    <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-4">
        <?php foreach ($displayProducts as $product): ?>
            <div class=" product-card2 flex flex-col items-center p-4 overflow-x-auto border rounded-lg hover:shadow-md transition " data-id="<?= $product['pro_id']?>">
                <img
                    src="<?= html_escape($product['pro_image']) ?>"
                    alt="<?= html_escape($product['pro_name']) ?>"
                    class="w-full h-64 object-contain rounded-lg transition-transform duration-300 hover:scale-110">
