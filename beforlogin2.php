    ...
    <div class="container mx-auto px-6 flex items-center justify-between">
        ...
        <!-- <div class="flex items-center space-x-6">
            <a href="#" class="relative text-gray-700 hover:text-black">
                ...
                <span id="cart-count" class="absolute -top-1 -right-3 bg-red-600 text-white text-xs px-1.5 py-0.5 rounded-full"></span>
            </a>
        </div> -->
    </div>
</header>
...
<div class="max-w-7xl mx-auto p-6 flex flex-col md:flex-row gap-6 mt-12 ">
    ...
    <div class="w-full md:w-[500px] h-[500px] mt-[25px] flex items-center justify-center bg-gray-100 rounded-lg overflow-hidden">
        <img id="mainImage" src="<?php echo $p_data['pro_image']; ?>" alt="Product Image"
...
loadAvgRating();
 ...
 <!-- #region -->
		...
		<h2 id="pro_name" class="text-2xl mt-4 mb-6 font-bold text-gray-800">Customer Review</h2>
		<div class="max-h-[350px] overflow-y-scroll pr-2 space-y-4">
		<?php foreach ($rating as $item): ?>
				<div class="p-4 mb-4 bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow transition duration-200">
					<div class="flex items-start gap-4">
						...
						<div class="flex-1">
							...
							<p class="text-gray-600 mt-1 text-sm"><?= htmlspecialchars($item['pro_comment']) ?></p>
						</div>
					</div>
				</div>
			<?php endforeach; ?>
	...
	$(document).ready(function(){
		...
		$("#wishlist").click(function(){
			...
			$.ajax({
				...
				dataType:"json",
				success: function(response) {
                console.log("Response:", response);image/sccategory/image (3).png
                if (response.status == "success") {
